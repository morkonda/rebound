...
double phase(){
	double dx = particles[0].x - particles[1].x;	
	double dy = particles[0].y - particles[1].y;	
	return atan2(dx,dy);
}
...
void problem_finish(){
	...
	fprintf(of,"%e\t%e\t%e\t%e\n",timing,rel_energy,dt_init,fabs(phase()-phase_init));
	fclose(of);
